...
import { addRole } from '../../../services/manageRolesService';
import ToggleSwitch from './ToggleSwitch';
import { showSuccessToast, showErrorToast } from '../../../utils/showToast';

export default function AddNewRoleModal({ open, onClose }) {
  ...
  const { mutate: submitAddRole, isPending } = useMutation({
    mutationFn: () =>
      addRole({
        name: roleName.trim(),
    ...
    onError: (error) => {
      showErrorToast(error?.response?.data?.Message ?? 'Failed to add role');
    },
  });
  if (!open) return null;
      ...
      >
        ...
        >
          <h2 className='mr-[29px] w-[95%] font-semibold text-[18px] leading-6 text-[#333]'>
            {MANAGE_ROLES_MODAL_TITLES.ADD_NEW_ROLE}
          </h2>
