...
export default function EditInternalOrderModal({
  ...
  useEffect(() => {
    if (!open) return;
    setStep(1);
    setFromInventory(detail?.fromInventory ?? order?.fromInventory ?? null);
    setToInventory(detail?.toInventory ?? order?.toInventory ?? null);
  ...
  useEffect(() => {
    if (step !== 2 || products.length === 0 || !detail) return;

    // Only pre-fill on first entry to step 2
    if (selectedItems.length > 0) return;

    const detailProducts = detail?.data?.flatMap((g) => g.products) ?? [];
    ...
    detailProducts.forEach((dp) => {
      const match = products.find((p) =>
        (p.products || []).some(
          (batch) => batch.storeProductId === dp.storeProductItemId,
        ),
      );
      if (match) {
        preSelected.push(match);
        preQuantities[match.id] = dp.orderQuantity;
      }
    });
              ...
              >
                ...
                <div className='px-6 mb-0' ref={searchRef}>
                  <div className='relative'>
                    <div
                      className={`flex items-center border rounded px-3 gap-2 transition-colors ${searchFocused ? 'border-emerald-500 ring-1 ring-emerald-500' : 'border-gray-300'}`}
                    >
                          ...
                          <div className='flex flex-col gap-1.5'>
                            {filteredProducts.map((product) => {
                              ...
                              return (
                                ...
                                >
                                  <div
                                    className={`w-4 h-4 rounded flex items-center justify-center border shrink-0 ${sel ? 'bg-emerald-600 border-emerald-600' : 'border-gray-300 bg-white'}`}
                                  >
                                    {sel && (
                                      <svg
